      ...
      <div className="container pb-10">
        <div className="bg-card rounded-2xl shadow-2xl overflow-hidden grid md:grid-cols-[1fr_1.2fr_1fr] gap-0">
          ...
          <div className="p-6 flex items-start justify-center bg-card">
            <div className="w-full aspect-square rounded-lg border border-border flex items-center justify-center overflow-hidden"
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="flex items-center gap-2 text-xs">
              <span className="bg-secondary text-foreground px-2 py-0.5 rounded-full font-medium">0%</span>
              <span className="text-muted-foreground">just now</span>
            </div>

            <div className="flex flex-wrap gap-4 text-sm">
              <div className="flex items-center gap-2"><Truck className="h-4 w-4 text-amber-500" /> Pan India Delivery</div>
              <div className="flex items-center gap-2"><Wallet className="h-4 w-4 text-emerald-500" /> Get Credit</div>
            </div>
            ...
            {(selection.colorCode || selection.colorName || selection.colorHex) && (
              <Field label={selection.colorSystem || "Color"}>
                <div className="inline-flex items-center gap-2 border border-border rounded-md px-2.5 py-1">
                  {selection.colorHex && (
                    <span className="h-4 w-4 rounded-sm border border-border" style={{ backgroundColor: selection.colorHex }} />
                  )}
                  <span className="text-sm">
                    {[selection.colorCode, selection.colorName].filter(Boolean).join(" — ")}
                    {selection.colorHex && <span className="text-muted-foreground ml-2">{selection.colorHex.toUpperCase()}</span>}
                  </span>
                </div>
              </Field>
            )}
            ...
            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 flex items-center justify-between gap-3">
              ...
              <Link to="/" className="text-sm font-semibold text-primary hover:underline">Click Here</Link>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 bg-card border-l border-border space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={sendOtp} className="absolute right-2 top-1/2 -translate-y-1/2 text-sm font-semibold text-primary">
                  {otpSent ? "Resend" : "Get OTP"}
                </button>
              </div>
            </div>

            <FormInput label="OTP" value={otp} onChange={(v) => setOtp(v.replace(/\D/g, "").slice(0, 6))} placeholder="Enter OTP" />
